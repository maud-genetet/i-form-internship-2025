...
class ToolbarManager:
    """Manages all toolbar components and user controls"""
    ...
    def _create_top_toolbar(self, main_layout):
        """Create top toolbar with data info and navigation"""
    ...
    def _add_visualization_options_section(self, toolbar_layout):
        """Add visualization option controls"""
        ...
        self.constraint_size_spinbox.valueChanged.connect(
            self._on_constraint_size_changed)
        toolbar_layout.addWidget(self.constraint_size_spinbox)

        # Monochromatic mode
        self.monochromatic_checkbox = QCheckBox("Monochromatic")
        self.monochromatic_checkbox.toggled.connect(
            self._on_monochromatic_toggled)
        ...
        self.line_contour_checkbox.toggled.connect(
            self._on_line_contour_toggled)
        ...
        self.vector_size_spinbox.valueChanged.connect(
            self._on_vector_size_changed)
        ...
        self.remove_variables_btn.clicked.connect(
            self._on_remove_variables_clicked)
        toolbar_layout.addWidget(self.remove_variables_btn)

    def _create_bottom_toolbar(self, main_layout):
        """Create bottom toolbar with visualization controls"""
    ...
    def _on_remove_variables_clicked(self):
        """Handle remove variables action"""
    ...
    def disable_auto_scale_during_loading(self):
        """Disable auto-scale during file loading"""
    ...
    def _refresh_display(self):
        """Refresh visualization when options change"""
        if hasattr(self.main_window, 'field_variables_handler'):
    ...
    def show_navigation_controls(self, neu_files):
        """Show navigation controls for multi-file datasets"""
