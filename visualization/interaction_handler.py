...
class InteractionHandler:
    ...
    def enable_mesh_picking(self):
        ...
        logger.info(
            f"Enabling picking on mesh with {self.current_mesh.n_cells} cells")
        ...
        interactor.add_observer('LeftButtonPressEvent',
                                self._vtk_click_handler)
        ...
        try:
            ...
            if cell_result:
                ...
                distances = np.linalg.norm(
                    points - np.array(world_pos), axis=1)
                ...
                if min_distance <= distance_threshold:
                    ...
                    if self.info_content:
                        self.info_content.setText(
                            "No node found at click position")
            else:
                if self.info_content:
                    self.info_content.setText(
                        "No mesh found at click position")
    ...
    def _highlight_picked_node(self, point_id):
        ...
        if self.current_mesh and point_id < self.current_mesh.n_points:
            ...
            sphere_radius = self._calculate_proportional_size(
                self.current_mesh, base_factor=0.011)
    ...
    def go_to_point_coordinates(self, target_point):
        ...
        try:
            ...
            target_pos = np.array(
                [target_point[0], target_point[1], target_point[2] if len(target_point) > 2 else 0])
