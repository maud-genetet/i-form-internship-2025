...
class DisplayModeManager:
    ...
    def display_mesh(self, plotter, mesh, mesh_color, edge_color, show_edges=True):
        ...
        if 'Material_Colors' in mesh.cell_data:
            ...
            if self.wireframe_mode:
                plotter.add_mesh(mesh, style='wireframe', scalars='Material_Colors',
                                 rgb=True, line_width=1, label="Mesh - Materials (Wireframe)")
            else:
    ...
    def display_variable_with_options(self, plotter, mesh, scalar_name, variable_name, edge_color, options):
        ...
        if vector_mode:
            ...
            self._display_contours(
                plotter, mesh, scalars_array, variable_name, cmap)
        else:
            if wireframe_mode:
                plotter.add_mesh(
                    mesh,
    ...
    def _add_all_constraints(self, plotter, mesh):
        ...
        if hasattr(mesh, '_size_options') and mesh._size_options:
            constraint_size *= mesh._size_options.get(
                'constraint_size_factor', 1.0)
    ...
    def _display_vectors(self, plotter, mesh, scalar_name, variable_name):
        ...
        try:
            if "Velocity" in variable_name:
                ...
                vectors = self._calculate_vectors_from_variable(
                    mesh, scalar_name, variable_name)
            ...
            if vectors is not None and len(vectors) == len(points):
                ...
                if np.any(non_zero_mask):
                    ...
                    if hasattr(mesh, '_size_options'):
                        scale_factor *= mesh._size_options.get(
                ...
                if "Velocity" in variable_name:
                    if 'Velocity X(r)' in mesh.cell_data and 'Velocity Y(z)' in mesh.cell_data:
                        ...
                        vectors = np.column_stack(
                            [vel_x, vel_y, np.zeros_like(vel_x)])
                        return vectors
                    ...
                    if 'Force X(r)' in mesh.cell_data and 'Force Y(z)' in mesh.cell_data:
                        ...
                        vectors = np.column_stack(
                            [force_x, force_y, np.zeros_like(force_x)])
                        return vectors
                return None
    ...
    def _add_constraints_visualization(self, plotter, mesh):
        ...
        self._create_batched_constraint_spheres(
            plotter, constraint_groups, constraint_size, subdivisions)

        # Add contact nodes
        self._add_contact_nodes_visualization(
            plotter, mesh, constraint_size, subdivisions)
