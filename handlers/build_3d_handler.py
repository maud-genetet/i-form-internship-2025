...
class Build3DHandler:
        ...
        for i_div in range(divisions):
            for elem_2d in elements_2d:
                ...
                if i_div == divisions - 1 and model_type in ["axisymmetric"]:
                    ...
                    node5_3d_id = nodes_2d[0].get_id() + (i_div + 1) * nodes_2d_count
                    node6_3d_id = nodes_2d[1].get_id() + (i_div + 1) * nodes_2d_count
                    node7_3d_id = nodes_2d[2].get_id() + (i_div + 1) * nodes_2d_count
                    node8_3d_id = nodes_2d[3].get_id() + (i_div + 1) * nodes_2d_count
    ...
    def _copy_element_properties_3d(self, elem_2d, elem_3d, model_type, i_div, divisions, params):
        ...
        if model_type in ["axisymmetric", "axisymmetric_cheese"]:
            ...
            alpha = (angle_total * (i_div + 0.5)) / divisions  # Mid-element angle
            ...
            strain_x = strain_r * math.cos(alpha)**2 + strain_t * math.sin(alpha)**2
            strain_y = strain_r * math.sin(alpha)**2 + strain_t * math.cos(alpha)**2
            strain_xy = (strain_r - strain_t) * math.cos(alpha) * math.sin(alpha)
            ...
            stress_x = stress_r * math.cos(alpha)**2 + stress_t * math.sin(alpha)**2
            stress_y = stress_r * math.sin(alpha)**2 + stress_t * math.cos(alpha)**2
            stress_xy = (stress_r - stress_t) * math.cos(alpha) * math.sin(alpha)
    ...
    def setupUi(self):
        ...
        if self.model_type in ["axisymmetric", "axisymmetric_cheese"]:
            ...
            if self.model_type == "axisymmetric_cheese":
                ...
                angle_layout.addWidget(QLabel("Angle of the Rotational Cut (Degrees):"))
